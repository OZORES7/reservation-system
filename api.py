...
def seed_data(conn: sqlite3.Connection) -> None:
    ...
    if movie_count == 0:
        movies = [
            ("Barbie", "Fantasy comedy", 114, "Comedy", "English", "2023-07-21", "img/home1.jpg", "now_showing"),
            ("Oppenheimer", "Biographical drama", 180, "Drama", "English", "2023-07-21", "img/home2.jpg", "now_showing"),
            ("Spider-Man: No Way Home", "Marvel action", 148, "Action", "English", "2021-12-17", "img/home3.jpg", "now_showing"),
        ]
        conn.executemany(
            """
    ...
    screen = conn.execute("SELECT id FROM screens WHERE theater_id = ? LIMIT 1", (theater_id,)).fetchone()
    if not screen:
        conn.execute(
    ...
    if seat_count == 0:
        ...
        for row_label in "ABCDEFGHIJ":
            for seat_number in range(1, 13):
                seat_rows.append((screen_id, f"{row_label}{seat_number}", row_label, seat_number, "regular", 1))
        conn.executemany(
            """
    ...
    if showtime_count == 0:
        ...
        for idx, movie in enumerate(movies):
            ...
            showtimes.append((movie["id"], screen_id, show_start.isoformat(), show_end.isoformat(), 10.0, "scheduled"))
        conn.executemany(
            """
...
def signup(payload: SignupRequest) -> dict[str, Any]:
    with get_conn() as conn:
        exists = conn.execute(
            "SELECT id FROM users WHERE username = ? OR email = ?", (payload.username, payload.email)
        ).fetchone()
        if exists:
...
def get_showtime_seats(showtime_id: int) -> dict[str, Any]:
    with get_conn() as conn:
        showtime = conn.execute(
            "SELECT id, screen_id FROM showtimes WHERE id = ?", (showtime_id,)
        ).fetchone()
        if not showtime:
